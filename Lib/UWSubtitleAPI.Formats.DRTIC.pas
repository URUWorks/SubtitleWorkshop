...
function TUWDRTIC.SaveSubtitle(const FileName: String; const FPS: Single; const Encoding: TEncoding; const Subtitles: TUWSubtitles; const FromItem: Integer = -1; const ToItem: Integer = -1): Boolean;
var
...
begin
  ...
  try
    SubFile.Add(SysUtils.Format('[JRT2: %d 0 ]', [Subtitles.Count]), False);
    ...
    begin
      DecimalSep       := DecimalSeparator;
      DecimalSeparator := '.';
      SubFile.Add(SysUtils.Format('{1}{1}%.3f', [FPS]), False);
      DecimalSeparator := DecimalSep;
    end;
    ...
    begin
      ...
      Subtitles.Text[i] := SWTagsToMicroDVD(Subtitles.Text[i]);
      SubFile.Add('{' + IntToStr(TimeToFrames(Subtitles[i].InitialTime, FPS)) + '}{' + IntToStr(TimeToFrames(Subtitles[i].FinalTime, FPS)) + '}' +
...
function TUWDRTIC.ToText(const Subtitles: TUWSubtitles): String;
begin
  Result := '';
end;
